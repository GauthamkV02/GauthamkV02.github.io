...
Bloom.prototype = {
	draw: function () {
		...
		for (var i = 0; i < this.petals.length; i++) {
			...
			finished &= this.petals[i].isfinished;
		}
...
Garden.options = {
	...
	growFactor: { min: 0.5, max: 1.2 },
	bloomRadius: { min: 10, max: 16 },
	growSpeed: 1000 / 60,
	color: {
		rmin: 180, rmax: 255,
		gmin: 80,  gmax: 220,
...
Garden.random = (min, max) => Math.random() * (max - min) + min;
Garden.randomInt = (min, max) => Math.floor(Math.random() * (max - min + 1)) + min;
Garden.circle = Math.PI * 2;
Garden.degrad = angle => Garden.circle / 360 * angle;
Garden.rgba = (r, g, b, a) => `rgba(${r},${g},${b},${a})`;
Garden.randomrgba = (rmin, rmax, gmin, gmax, bmin, bmax, a) =>
	Garden.rgba(
		Math.round(Garden.random(rmin, rmax)),
		Math.round(Garden.random(gmin, gmax)),
		Math.round(Garden.random(bmin, bmax)),
		a
	);
